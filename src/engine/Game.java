...
public class Game {
	...
	public boolean callHasEffect(Champion currentChampion, String s) {
		return hasEffect(currentChampion, s);
	}
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH)
				...
				if (board[currx][curry] instanceof Cover) {
					...
					//TODO should decrement action points
	...
	public void useLeaderAbility() throws LeaderNotCurrentException, LeaderAbilityAlreadyUsedException {
		...
		//TODO comment the line below so that leader does not apply effect on himself twice	
			targets.add(getCurrentChampion());
		//
			for (Champion c : team)
				targets.add(c);
		...
		//TODO
		ArrayList<Damageable> damageableTargets = new ArrayList<Damageable>();
		for(Champion c: targets) {
			damageableTargets.add((Damageable)c);
		}
		cleanup(damageableTargets);
		//
	...
	public void endTurn() {
		...
		while (!turnOrder.isEmpty() && hasEffect((Champion) turnOrder.peekMin(), "Stun")) {
			...
			//TODO what if all champions have stun effect, end turn should still happen until Stun duration is ended
			// or AntiHero useLeaderAbility used gives index out of bounds
			if(turnOrder.isEmpty()) {
				prepareChampionTurns();
			}
			// added by me
	...
	public void callPrepareChampionTurns() {
		prepareChampionTurns();
	}
